...
class Command(BaseCommand):
    help = 'Report users to expire in NIS'
    ...
    def handle(self, *args, **options):
        ...
        if options['sync']:
            self.sync = True
            logger.warn("Syncing NIS with ColdFront")
        ...
        # Print users whose latest allocation expiration date GTE 365 days and active in NIS
        for key in expired_allocation_users.keys():
            ...
            try:
                ...
                if 'nsAccountLock' in result and str(result['nsAccountLock'][0]).lower() == 'true':
                    ...
                    # User is active in NIS but not on any active allocations
                    self.writerow([
                        key,
